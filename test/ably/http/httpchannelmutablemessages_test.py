...
class TestHttpChannelMutableMessages(BaseAsyncTestCase):
    ...
    async def test_update_message_with_params(self):
        """Test updating a message with query parameters"""
        channel = self.ably.channels[self.get_channel_name('mutable:update_params')]

        # Publish message
        result = await channel.publish('test-event', 'original')
        assert len(result.serials) > 0
        ...
        operation = MessageOperation(description='Test with params')
        params = {'testParam': 'value'}

        update_result = await channel.update_message(message, operation, params)
        assert update_result is not None
